...
def train(config):
	...
	dev_dataset, dev_raw_label = load_train_dataset('train', revision, tokenizer) 		# validation용 데이터는 따로 만드셔야 합니다.
	...
	training_args = TrainingArguments(
		...
		per_device_train_batch_size=config.dataloader['args']['batch_size'],  	# batch size per device during training
		per_device_eval_batch_size=config.dataloader['args']['batch_size'],   	# batch size for evaluation
		warmup_steps=config.lr_scheduler['args']['warmup_steps'],               # number of warmup steps for learning rate scheduler
		weight_decay=config.optimizer['args']['weight_decay'],               	# strength of weight decay
